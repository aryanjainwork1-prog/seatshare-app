...
import type { DriverProfile, Trip } from "@workspace/api-client-react";
      ...
      {item.type === "driver" && item.driver && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-semibold text-sm">{item.driver.user?.name ?? `Driver #${item.driver.userId}`}</p>
              <p className="text-xs text-muted-foreground">{item.driver.user?.phone ?? ""}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="bg-accent rounded p-2">
              <p className="font-bold text-primary">{item.driver.rating.toFixed(1)}</p>
            ...
            <div className="bg-accent rounded p-2 flex items-center justify-center">
              <Badge variant={item.driver.isVerified ? "default" : "outline"} className="text-[10px]">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <Badge variant="outline" className="text-[10px] mt-0.5">{item.trip.status}</Badge>
            </div>
          </div>
          <div className="text-xs space-y-1">
            <p className="text-muted-foreground">From: <span className="text-foreground">{item.trip.originAddress}</span></p>
            <p className="text-muted-foreground">To: <span className="text-foreground">{item.trip.destAddress}</span></p>
            <p className="text-muted-foreground">
              Seats: <span className="text-foreground">{item.trip.availableSeats}</span>
              {" · "}
              Fare: <span className="text-primary font-semibold">${item.trip.farePerSeat}/seat</span>
            </p>
          </div>
        </div>
      )}
...
export default function LiveMap() {
  ...
  const [liveLocations, setLiveLocations] = useState<Map<number, DriverLocation>>(new Map());
  const wsRef = useRef<WebSocket | null>(null);

  const driverParams = { isOnline: true, limit: 100 };
  const { data: driversData } = useListDriverProfiles(driverParams, {
    query: { queryKey: getListDriverProfilesQueryKey(driverParams), staleTime: 30_000 },
  });
  const tripParams = { status: "active", limit: 100 };
  const { data: tripsData } = useListTrips(tripParams, {
    query: { queryKey: getListTripsQueryKey(tripParams), staleTime: 30_000 },
  });
  ...
  // Merge live WS locations into the driver list
  const driversWithLoc: DriverWithLoc[] = drivers
    .map((d) => {
      ...
      return lat != null && lng != null ? { ...d, currentLat: lat, currentLng: lng } : null;
    })
    .filter((d): d is DriverWithLoc => d !== null);

  const driversInBounds: DriverWithLoc[] = bounds
    ? driversWithLoc.filter((d) => bounds.contains([d.currentLat, d.currentLng]))
    : driversWithLoc;
    ...
    <div className="relative h-full w-full flex">
      ...
      <div className="w-64 shrink-0 border-r border-border bg-card flex flex-col z-[500] overflow-hidden">
        <div className="p-3 border-b border-border flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-[10px]">{driversInBounds.length}</Badge>
        </div>
        <ScrollArea className="flex-1">
          {driversInBounds.length === 0 ? (
            <p className="text-xs text-muted-foreground p-3">No online drivers in view</p>
          ) : (
            driversInBounds.map((d) => (
              <button
                key={d.id}
                onClick={() => setSelected({ type: "driver", driver: d })}
                className="w-full text-left p-3 hover:bg-accent transition-colors border-b border-border/50 last:border-0"
              >
                <p className="text-sm font-medium truncate">{d.user?.name ?? `Driver #${d.userId}`}</p>
                <p className="text-xs text-muted-foreground">⭐ {d.rating.toFixed(1)} · {d.totalTrips} trips</p>
      ...
      <div className="flex-1 relative">
        ...
        >
          ...
          {driversWithLoc.map((d) => (
            <Marker
              ...
              icon={DRIVER_ICON}
              eventHandlers={{ click: () => setSelected({ type: "driver", driver: d }) }}
            >
              <Popup>
                <div className="text-sm">
                  <strong>{d.user?.name ?? `Driver #${d.userId}`}</strong>
          ...
          {trips.map((t) => (
            <Marker
              ...
              icon={TRIP_ICON}
              eventHandlers={{ click: () => setSelected({ type: "trip", trip: t }) }}
            >
              <Popup>
                <div className="text-sm">
                  <strong>Trip #{t.id}</strong>
                  <br />{t.originAddress} → {t.destAddress}
                  <br />{t.availableSeats} seats · ${t.farePerSeat}/seat
                </div>
              </Popup>
            </Marker>
          ))}
        ...
        <div className="absolute top-3 right-3 z-[1000] flex items-center gap-1.5 bg-card/90 backdrop-blur-sm px-2.5 py-1.5 rounded-full border border-border text-xs">
          {wsStatus === "connected" ? (
            ...
            <><Wifi className="h-3 w-3 text-yellow-400 animate-pulse" /><span className="text-yellow-400">Connecting</span></>
          ) : (
      ...
      <div className="w-64 shrink-0 border-l border-border bg-card flex flex-col z-[500] overflow-hidden">
        <div className="p-3 border-b border-border flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-[10px]">{tripsInBounds.length}</Badge>
        </div>
        <ScrollArea className="flex-1">
          {tripsInBounds.length === 0 ? (
            <p className="text-xs text-muted-foreground p-3">No active trips in view</p>
          ) : (
            tripsInBounds.map((t) => (
              <button
                key={t.id}
                onClick={() => setSelected({ type: "trip", trip: t })}
                className="w-full text-left p-3 hover:bg-accent transition-colors border-b border-border/50 last:border-0"
              >
                <p className="text-sm font-medium">Trip #{t.id}</p>
                <p className="text-xs text-muted-foreground truncate">
                  {t.originAddress.split(",")[0]} → {t.destAddress.split(",")[0]}
                </p>
